...
<h2>YAML 字段 → Agent 配置</h2>
<table class="t">
  <tr><th>YAML 字段</th><th>对应 Agent 配置</th><th>说明</th></tr>
  <tr><td class="mono">name</td><td class="mono">Agent(name=…)</td><td>Agent 名字</td></tr>
  <tr><td class="mono">instructions</td><td class="mono">Agent(instructions=…)</td><td>系统提示</td></tr>
  <tr><td class="mono">model</td><td>ChatClient 的模型名</td><td>如 gpt-4o</td></tr>
  <tr><td class="mono">tools</td><td class="mono">Agent(tools=[…])</td><td>按名字解析（可配 bindings）</td></tr>
  <tr><td class="mono">middleware</td><td class="mono">Agent(middleware=[…])</td><td>中间件列表</td></tr>
  <tr><td class="mono">context_providers</td><td class="mono">Agent(context_providers=[…])</td><td>上下文 / 记忆提供者</td></tr>
</table>
...
<h2>加载流水线</h2>
<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>读取 YAML</h4>
    <p><span class="mono">create_agent_from_yaml_path("my_agent.yaml")</span> 读入配置文件。</p></div></div>
  <div class="step"><div class="num">2</div><div class="sc"><h4>解析 + 校验</h4>
    <p><span class="mono">AgentFactory</span> 按已知 schema 解析；未知字段会报清晰的错误，而不是悄悄失败。</p></div></div>
  <div class="step"><div class="num">3</div><div class="sc"><h4>解析工具名</h4>
    <p>把 <span class="mono">tools</span> 里的名字通过 <span class="mono">bindings</span> 映射到真正的 Python 函数。</p></div></div>
  <div class="step"><div class="num">4</div><div class="sc"><h4>构造 Agent</h4>
    <p>吐出一个和手写 <span class="mono">Agent(client=…, tools=…)</span> 完全同型的实例，<span class="mono">run()</span> 照常用。</p></div></div>
</div>
...
<h2>YAML fields → Agent config</h2>
<table class="t">
  <tr><th>YAML field</th><th>Maps to Agent config</th><th>Notes</th></tr>
  <tr><td class="mono">name</td><td class="mono">Agent(name=…)</td><td>agent name</td></tr>
  <tr><td class="mono">instructions</td><td class="mono">Agent(instructions=…)</td><td>system prompt</td></tr>
  <tr><td class="mono">model</td><td>the ChatClient's model</td><td>e.g. gpt-4o</td></tr>
  <tr><td class="mono">tools</td><td class="mono">Agent(tools=[…])</td><td>resolved by name (via bindings)</td></tr>
  <tr><td class="mono">middleware</td><td class="mono">Agent(middleware=[…])</td><td>middleware list</td></tr>
  <tr><td class="mono">context_providers</td><td class="mono">Agent(context_providers=[…])</td><td>context / memory providers</td></tr>
</table>
...
<h2>The load pipeline</h2>
<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>Read the YAML</h4>
    <p><span class="mono">create_agent_from_yaml_path("my_agent.yaml")</span> reads the config file.</p></div></div>
  <div class="step"><div class="num">2</div><div class="sc"><h4>Parse + validate</h4>
    <p><span class="mono">AgentFactory</span> parses against a known schema; unknown fields raise a clear error instead of failing silently.</p></div></div>
  <div class="step"><div class="num">3</div><div class="sc"><h4>Resolve tool names</h4>
    <p>Names under <span class="mono">tools</span> are mapped to real Python functions via <span class="mono">bindings</span>.</p></div></div>
  <div class="step"><div class="num">4</div><div class="sc"><h4>Construct the Agent</h4>
    <p>Out comes an instance identical to a hand-written <span class="mono">Agent(client=…, tools=…)</span>; <span class="mono">run()</span> works as usual.</p></div></div>
</div>
...
<h2>三层中间件（洋葱由外到内）</h2>
<div class="layers">
  <div class="layer l-core"><div class="lh"><span class="badge">最外层</span><span class="name">AgentMiddleware</span></div>
    <div class="ld">包住<strong>整轮 run()</strong>：context 是 <span class="mono">AgentContext</span>，能看到输入消息和最终 <span class="mono">AgentResponse</span>。</div></div>
  <div class="layer l-main"><div class="lh"><span class="badge">中间层</span><span class="name">FunctionMiddleware</span></div>
    <div class="ld">包住<strong>每次工具调用</strong>：context 是 <span class="mono">FunctionInvocationContext</span>，能看到函数名、参数和返回值。</div></div>
  <div class="layer l-part"><div class="lh"><span class="badge">最内层</span><span class="name">ChatMiddleware</span></div>
    <div class="ld">包住<strong>每次 LLM 调用</strong>：context 是 <span class="mono">ChatContext</span>，最贴近模型，能看到发出的消息和 <span class="mono">ChatResponse</span>。</div></div>
</div>
<p>每一层都实现 <span class="mono">process(self, context, call_next)</span>：<span class="mono">await call_next()</span> <strong>之前</strong>是"进站"、<strong>之后</strong>是"出站"——无参，数据全走 <span class="mono">context</span>。</p>

<h2>三类中间件拦截什么</h2>
<table class="t">
  <tr><th>中间件</th><th>拦截的范围</th><th>context 类型</th><th>触发时机</th></tr>
  <tr><td class="mono">AgentMiddleware</td><td>整轮 agent.run()</td><td class="mono">AgentContext</td><td>每次 run，一次</td></tr>
  <tr><td class="mono">FunctionMiddleware</td><td>单次工具 / 函数调用</td><td class="mono">FunctionInvocationContext</td><td>每调用一个工具</td></tr>
...
<h2>Three middleware layers (onion, outside in)</h2>
<div class="layers">
  <div class="layer l-core"><div class="lh"><span class="badge">outermost</span><span class="name">AgentMiddleware</span></div>
    <div class="ld">Wraps the <strong>whole run()</strong>: context is <span class="mono">AgentContext</span>, seeing input messages and the final <span class="mono">AgentResponse</span>.</div></div>
  <div class="layer l-main"><div class="lh"><span class="badge">middle</span><span class="name">FunctionMiddleware</span></div>
    <div class="ld">Wraps <strong>each tool call</strong>: context is <span class="mono">FunctionInvocationContext</span>, seeing the function name, arguments and result.</div></div>
  <div class="layer l-part"><div class="lh"><span class="badge">innermost</span><span class="name">ChatMiddleware</span></div>
    <div class="ld">Wraps <strong>each LLM call</strong>: context is <span class="mono">ChatContext</span>, closest to the model, seeing the messages sent and the <span class="mono">ChatResponse</span>.</div></div>
</div>
<p>Every layer implements <span class="mono">process(self, context, call_next)</span>: before <span class="mono">await call_next()</span> is the "inbound" half, after it is the "outbound" half — no args, all data flows through <span class="mono">context</span>.</p>

<h2>What each middleware kind intercepts</h2>
<table class="t">
  <tr><th>Middleware</th><th>Intercepts</th><th>Context type</th><th>Fires</th></tr>
  <tr><td class="mono">AgentMiddleware</td><td>the whole agent.run()</td><td class="mono">AgentContext</td><td>once per run</td></tr>
...
<h2>检查点存储后端</h2>
<table class="t">
  <tr><th>存储</th><th>持久性</th><th>适用场景</th><th>来自</th></tr>
  <tr><td class="mono">InMemoryCheckpointStorage</td><td>进程内（重启即失）</td><td>开发 / 测试</td><td>核心包</td></tr>
  <tr><td class="mono">FileCheckpointStorage</td><td>本地文件（持久）</td><td>单机生产 / 零依赖</td><td>核心包</td></tr>
  <tr><td class="mono">CosmosCheckpointStorage</td><td>云端分布式（强一致）</td><td>分布式生产 / 高可用</td><td class="mono">agent-framework-azure-cosmos</td></tr>
</table>
...
<h2>工具审批（HITL）流程</h2>
<div class="flow">
  <div class="node"><div class="nt">模型想调工具</div><div class="nd">如 publish_article</div></div>
  <div class="arrow">→</div>
  <div class="node"><div class="nt">approval_mode</div><div class="nd">always_require</div></div>
  <div class="arrow">→</div>
  <div class="node"><div class="nt">⏸ 暂停等人</div><div class="nd">请求人工确认</div></div>
  <div class="arrow">→</div>
  <div class="node hl"><div class="nt">✅ 批准 → 执行</div><div class="nd">❌ 拒绝 → 跳过</div></div>
</div>
...
<h2>崩溃恢复：存档怎么救回进度</h2>
<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>跑到 superstep 边界</h4>
    <p>Workflow 每跑完一个 superstep，就在边界把状态写入 <span class="mono">CheckpointStorage</span>。</p></div></div>
  <div class="step"><div class="num">2</div><div class="sc"><h4>💥 进程崩溃</h4>
    <p>机器挂了、部署重启、OOM……内存里的一切都没了。</p></div></div>
  <div class="step"><div class="num">3</div><div class="sc"><h4>↺ 从最近存档恢复</h4>
    <p>重启后从持久化后端读回最近一次 checkpoint，<strong>从断点继续</strong>，已完成的 superstep 不重跑。</p></div></div>
</div>
...
<h2>Checkpoint storage backends</h2>
<table class="t">
  <tr><th>Storage</th><th>Durability</th><th>Use case</th><th>From</th></tr>
  <tr><td class="mono">InMemoryCheckpointStorage</td><td>in-process (lost on restart)</td><td>dev / tests</td><td>core package</td></tr>
  <tr><td class="mono">FileCheckpointStorage</td><td>local file (persistent)</td><td>single-host prod / zero deps</td><td>core package</td></tr>
  <tr><td class="mono">CosmosCheckpointStorage</td><td>cloud distributed (strong consistency)</td><td>distributed prod / HA</td><td class="mono">agent-framework-azure-cosmos</td></tr>
</table>
...
<h2>Tool approval (HITL) flow</h2>
<div class="flow">
  <div class="node"><div class="nt">Model wants a tool</div><div class="nd">e.g. publish_article</div></div>
  <div class="arrow">→</div>
  <div class="node"><div class="nt">approval_mode</div><div class="nd">always_require</div></div>
  <div class="arrow">→</div>
  <div class="node"><div class="nt">⏸ Pause for human</div><div class="nd">request approval</div></div>
  <div class="arrow">→</div>
  <div class="node hl"><div class="nt">✅ Approve → run</div><div class="nd">❌ Reject → skip</div></div>
</div>
...
<h2>Crash recovery: how a checkpoint saves progress</h2>
<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>Run to a superstep boundary</h4>
    <p>After each superstep, the workflow writes state to <span class="mono">CheckpointStorage</span> at the boundary.</p></div></div>
  <div class="step"><div class="num">2</div><div class="sc"><h4>💥 Process crashes</h4>
    <p>The machine dies, a deploy restarts, OOM… everything in memory is gone.</p></div></div>
  <div class="step"><div class="num">3</div><div class="sc"><h4>↺ Resume from the latest checkpoint</h4>
    <p>On restart, load the most recent checkpoint from the persistent backend and <strong>continue from there</strong> — completed supersteps are not re-run.</p></div></div>
</div>
